...
package com.mvp.rxandroid.widgets.wheel;
...
//		setBackgroundResource(R.drawable.wheel_bg);
	...
	private int getMaxTextLength() {
		...
		for (int i = adapter.getItemsCount(); i > Math.max(adapter.getItemsCount() - addItems, 0); i--) {
			String text = adapter.getItem(i);
			if (text != null
					&& (maxText == null || maxText.length() < text.length())) {
				maxText = text;
	...
	private void drawShadows(Canvas canvas) {
		topShadow.setBounds(0, 1, getWidth(), getHeight()*3 / visibleItems);
		topShadow.draw(canvas);

		bottomShadow.setBounds(0, getHeight() - getHeight()*3 / visibleItems,
